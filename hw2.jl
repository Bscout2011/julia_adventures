...
# ╔═╡ f65f5aaa-1512-44e9-9363-21967b5a79f1
size(v)[1]
...
@bind window_l Slider(0:5, show_value=true)
...
function extend(M::AbstractMatrix, i, j)
	height, width = size(M)
	# easier to clamp i and j to in range
	i = clamp(i, 1, height)
	j = clamp(j, 1, width)
	return M[i, j]
end
...
# ╔═╡ 521eb35e-7629-4a94-b535-d665524ca77a
...
colored_line(box_blur(v, window_l))
...
function convolve(v::AbstractVector, k)
	l = (length(k)-1) ÷ 2
	koa = OffsetArray(k, -l:l)
	
	len_v = length(v)
	output = zeros(len_v)
	for i in 1:len_v
		for m in -l:l
			output[i] += extend(v, i+m) * koa[m]
		end
	end
...
function convolve(M::AbstractMatrix, K::AbstractMatrix)
	...
	Koa = OffsetArray(K, -l:l, -l:l)
	
	conv = [sum([extend(M, i+k, j+m) * Koa[k, m] 
				for k ∈ -l:l, m ∈ -l:l])
			for i ∈ 1:h, j ∈ 1:w]
...
# ╔═╡ 5e9b4bb2-12d4-4cfc-be9a-6ea56c281f34
begin
...
# ╔═╡ 641c79fe-eab7-459a-8325-ebf63e0a9491
Gx.^2 + Gy.^2

# ╔═╡ a9bd987c-726e-41de-80af-35a4eb597cd4
sobel_kernel()
...
function with_sobel_edge_detect(image)
	...
	conv_x = convolve(image, Gx)
	conv_y = convolve(image, Gy)
