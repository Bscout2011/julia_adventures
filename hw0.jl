...
student = (name = "Andrew Washburn", kerberos_id = "bscout@mit.edu")
...
ex_1_1 = md"""
The sqaure root finds the solution $a \sqrt x = x$, rewritten as $\sqrt x = x/a$. 

If $a < \sqrt x$, then $x/a > \sqrt x$, otherwise if $a > \sqrt x$, then $x/a < \sqrt x$. Therefore taking the average between $a$ and $x/a$ will reduce the bounds of $\sqrt x$.
""" 
...
function newton_sqrt(x, error_margin=0.01, a=x / 2) # a=x/2 is the default value of `a`
	x_0 = x
	while abs((x_0 / a) - a) > error_margin 
		x = x / a
		a = (x_0/a + a) / 2
	end
	return a
...
# ╔═╡ e3ec056d-7cc1-482a-a2bd-b466cdb275a6
